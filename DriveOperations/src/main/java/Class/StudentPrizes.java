...
public class StudentPrizes extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					StudentPrizes frame = new StudentPrizes();
	 ...
	 * Create the frame.
	 */
	public StudentPrizes(String prize_id) {
		setResizable(false);
	...
	JLabel lblajoutezLesClasses = new JLabel("Eleves choisis: "+ RolesAndPrizes.getPrizeName(prize_id));
	...
	JLabel lblajouterModifierouSupprimer = new JLabel("<html><div style='text-align: center;'>Ceci est la liste des eleves qui ont remporte ce prix ou ont ete choisis pour remporter ce titre.</div></html>");
	...
	loadStudents(prize_id);
	}

	

	
public static void loadStudents (String prize_id) {
		...
		try {
			...
			ResultSet rs=stmt.executeQuery("select * from student_prizes AS sr "
					+ "JOIN students AS s "
		...
		if(panel.getComponentCount() == 0) {
			...
			JLabel lblNewLabel = new JLabel("<html><div style='text-align: left;'>Aucun eleve n'a ete choisi pour remporter ce titre/prix. Cliquez sur \"Ajouter\" pour en ajouter.</div></html>");
